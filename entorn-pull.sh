...
if ! docker pull ${remote_base_name}-hub:$ver 2>/dev/null ; then
	error=true
fi
docker tag ${remote_base_name}-hub:$ver entorn-io/hub:${ver} 2>/dev/null
docker rmi ${remote_base_name}-hub:$ver 2>/dev/null
